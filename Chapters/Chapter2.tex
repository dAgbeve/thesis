...
\begin{figure}[h!]
    \begin{tikzpicture}
        ...
        \umlclass[x=4.0,y=-3]{SensorNode}{
            -m\_port : uint16\_t \\ -m\_PreState : uint16\_t \\ m\_received :
            uint64\_t \\ -m\_socket : Ptr\tiny{$\langle$} \small{Socket} \tiny{
            $\rangle$} \\ -m\_lossCounter : PacketLossCounter
        }{
        +SensorNode() \\
    ...
    \umlemptyclass[x=-4.5,y=-5,template={T,U}]{std::vector}{}{
    }
    \umlclass[x=5.5,y=-10]{SensorNodeHelper}{
        -m\_factory : ObjectFactory \\
        -m\_server : Ptr{\tiny$\langle$}TmpSense{\tiny$\rangle$}
    }
    {
        ...
        +GetServer(void) : Ptr{\tiny$\langle$}TmpSense{\tiny$\rangle$}
    }
